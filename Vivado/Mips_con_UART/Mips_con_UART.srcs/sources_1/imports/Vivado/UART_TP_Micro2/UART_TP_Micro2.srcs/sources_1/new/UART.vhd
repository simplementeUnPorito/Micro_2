...
architecture Behavioral of UART is
    ...
    signal en_tx_write_prev  : std_logic := '0';
    signal en_rx_read_prev   : std_logic := '0';
    signal en_tx_write_pulse : std_logic := '0';
    signal en_rx_read_pulse  : std_logic := '0';
...
begin
    ...
    RX_Block: entity work.reciever_fifo
        port map (
            ...
            en_data_out => en_rx_read_pulse,
            fifo_full   => rx_full_i,
            state       => state_Re
        );

    --------------------------------------------------------------------
    -- Flags y detección de flancos (1 ciclo)
    --------------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- Detectar flanco de subida y generar pulso de 1 ciclo
                if (en_tx_write = '1' and en_tx_write_prev = '0') then
                    en_tx_write_pulse <= '1';
                else
                    en_tx_write_pulse <= '0';
                end if;

                if (en_rx_read = '1' and en_rx_read_prev = '0') then
                    en_rx_read_pulse <= '1';
                else
                    en_rx_read_pulse <= '0';
                end if;

                -- Actualizar registros previos
                en_tx_write_prev <= en_tx_write;
                en_rx_read_prev  <= en_rx_read;
            end if;
        end if;
    end process;
